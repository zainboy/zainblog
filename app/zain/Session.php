...
class Session
{
    ...
    {
        if (empty($config)) {
            $config = \Zain\Config::get('session');
        }
        // 记录初始化信息
        \Zain\Config::get('APP_DEBUG') && zLogs('[ SESSION ] INIT ' . var_export($config, true));
        $isDoStart = false;
        ...
        if (!empty($config['type'])) {
            // 读取session驱动
            $class = false !== strpos($config['type'], '\\') ? $config['type'] : '\\Zain\\session\\driver\\' . ucwords($config['type']);
            // 检查驱动类
